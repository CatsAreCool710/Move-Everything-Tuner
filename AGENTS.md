...
```bash
# Build (Docker ARM64 cross-compilation)
...
- **JS UI** (QuickJS): Polls DSP via `host_module_get_param()`, drives 128x64 display, handles input, screen reader announcements.
...
src/
  ...
  dsp/
    ...
    tuner_audio.h/c    Audio feedback (step guide, reference tone, pluck)
...
- **C**: `snake_case`, `tuner_` prefix for public functions. Log: `g_host->log("[tuner] message")`.
- **JS**: `camelCase` for locals, `snake_case` for host API. Log: `console.log()`.
